...
switch ($action) {
	...
	case 'get_list' :
		...
		$rResult = mysql_query("SELECT   GROUP_CONCAT(client_loan_schedule.id) AS `id`,
                                ...
                                AND      DATE(client_loan_schedule.schedule_date) < '2017-12-31' AND (DATE_FORMAT(CURDATE(),'%m-%d') = '12-31' OR CURDATE() > '2017-12-31')
	...
	case 'disable':
		...
		$res_penalty = mysql_query("SELECT   DATEDIFF('2017-12-31',client_loan_schedule.schedule_date) AS datediff,
                        		             client_loan_schedule.schedule_date
                        		    FROM     client_loan_schedule
                        		    JOIN     client_loan_agreement ON client_loan_agreement.id = client_loan_schedule.client_loan_agreement_id
                        		    JOIN     client ON client.id = client_loan_agreement.client_id
		...
		if ($penalty>0) {
		    ...
		    if ($check == 0) {
    		    mysql_query("INSERT INTO `31_dec_penalty` 
                            			(`user_id`, `datetime`, `accrual_date`, `sclient_loan_schedule_id`, `penalty`) 
                            	  VALUES 
                            			('$user_id', NOW(), '2017-12-31', '$check_penalty[id]', '$penalty')");
		    }
        }
		break;
	default:
