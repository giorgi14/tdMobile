...
switch ($action) {
	...
	case 'save_transaction':
	    ...
	    if ($id == '') {
            Add($hidde_id, $month_fee, $root,  $percent, $penalti_fee, $diff, $type_id);
        }else{
            Save($id,$month_fee, $root,  $percent, $penalti_fee, $diff);
        }
	...
	case 'get_shedule':
		...
		$data = array('id' => $res[id],'pay_amount' => $res[pay_amount], 'root' => $res[root], 'percent' => $res[percent], 'penalty' => $res[penalty]);
...
function Add($hidde_id, $month_fee, $root, $percent, $penalti_fee, $diff, $type_id){
    
	$user_id	= $_SESSION['USERID'];
	mysql_query("INSERT INTO `money_transactions` 
                            (`datetime`, `user_id`, `client_loan_schedule_id`, `pay_datetime`, `pay_amount`, `pay_root`, `pay_percent`, `pay_penalty`, `diff`, `type_id`, `status`, `actived`) 
		              VALUES 
						      (NOW(), '$user_id', '$hidde_id', curdate(), '$month_fee', '$root', '$percent', '$penalti_fee', '$diff', '$type_id', '1','1');");
}

function Save($id,$month_fee, $root,  $percent, $penalti_fee, $diff){
	    	...
	    	<table class="dialog-form-table">
    			...
    			<table>
    				...
    				<tr style="'.$input_hidde.'">
    					...
    					<td style="width: 180px;">
    						<input style="width: 80px;" id="root" class="label_label" type="text" value="'.$res['pay_root'].'">
    					</td>
    				...
    				<tr style="'.$input_hidde.'">
    					...
    					<td style="width: 180px;">
    						<input style="width: 80px;" id="percent" class="label_label" type="text" value="'.$res['pay_percent'].'">
    					</td>
    				...
    				<tr style="'.$input_hidde.'">
    					...
    					<td style="width: 180px;">
    						<input class="label_label" style="width: 80px;" id="penalti_fee" type="text" value="'.$res['pay_penalty'].'">
    					</td>
