...
switch ($action) {
    ...
    case 'get_list' :
		...
		$rResult = mysql_query("SELECT   client.id,
                        				 ...
                        				 ROUND((SELECT SUM(money_transactions.pay_percent)+SUM(pay_penalty) AS pay_percent
        								 FROM  `money_transactions`
                                ...
                                WHERE    client_loan_schedule.`status` = 1 AND client_loan_schedule.actived = 1 AND DATE_FORMAT(client_loan_schedule.pay_date,'%m') = $filt_month
