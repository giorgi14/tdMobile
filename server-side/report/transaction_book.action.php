...
// 	        break;
	case 'get_list' :
		...
		$rResult = mysql_query("SELECT 	MAX(client_loan_schedule.id),
		                        ...
		                        AND      client.actived = 1  AND client_loan_agreement.actived=1 AND client_loan_agreement.canceled_status=0
...
function client($id){
    $req = mysql_query("SELECT    cl.id,
                                  CASE
                        			  WHEN cl.attachment_id = 0 AND cl.`name` != '' THEN concat(client_loan_agreement.oris_code, '/', cl.`name`, ' ', cl.lastname)
                                      WHEN cl.attachment_id = 0 AND cl.`name` = '' THEN concat(client_loan_agreement.oris_code, '/', cl.ltd_name)
                                      WHEN cl.attachment_id != 0 AND cl.`name` = '' THEN concat(client_loan_agreement.oris_code, '/', cl.ltd_name)
                                      WHEN cl.attachment_id != 0 AND cl.`name` != '' THEN concat(client_loan_agreement.oris_code, '/', cl.`name`, ' ', cl.lastname, '/დანართი N', client_loan_agreement.attachment_number)
                                  END AS `name`
                        FROM      client AS cl
...
function GetPage($id){
    ...
    $res1= mysql_fetch_assoc(mysql_query("SELECT      client_loan_agreement.id AS agr_id,
	            ...
	            <table>
    				...
    				<tr>
    					...
    					<td style="width: 120px;">
    						<select id="client_loan_number" calss="label" style="width: 175px;" disabled="disabled">'.client_loan_number($res1[agr_id]).'</select>
    					</td>
    				</tr>
    				...
    				<tr style="display:none">
    					...
    					<td style="width: 280px;">
    						<input class="idle" style="width: 15px;" id="" value="1" type="checkbox">
    					</td>
    					<td style="width: 120px;">
    					</td>
    				</tr>
